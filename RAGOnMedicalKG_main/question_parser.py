...
class QuestionPaser:
    '''构建实体节点 得到{类别: 相应实体list}'''
    def build_entitydict(self, args): # args: {问句中的关键词: 关键词所属类别}
        entity_dict = {}
        ...
        return entity_dict # {类别：相应实体list}

    '''解析主函数'''
    def parser_main(self, res_classify):
        """
        input: res_classify 
                includes: args {问句中的关键词: 关键词所属类别} (dict)
                          question_types (list)
        output: dict [question_type: sql语句list]
        """
    ...
    def sql_transfer(self, question_type, entities):
        ...
        if question_type == 'disease_cause':
            sql = ["MATCH (m:Disease) where m.name = '{0}' return m.name, m.cause".format(i) for i in entities] #用于找到 Disease 节点,其 name 属性等于当前遍历到的 i 值。
